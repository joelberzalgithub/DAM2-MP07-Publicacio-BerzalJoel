...
class AppData with ChangeNotifier {
  ...
  Future<void> addDemons() async {
    ...
    await database.transaction((txn) async {
      for (int id = 1; id < 13; id++) {
        List<String> demonClasses = [];
        List<String> demonRanks = [];
        List<int> demonHealths = [];
        List<int> demonDamages = [];
        List<String> demonDescriptions = [];
        List<String> demonImages = [];
        List<String> demonNames = [];
        
        List<Map<String, dynamic>> demon = await txn.rawQuery(
          'SELECT class, rank, health_points, damage, description '
          'FROM demons '
          'WHERE id = $id'
        );

        for (Map<String, dynamic> row in demon) {
          if (!demonClasses.contains(row['class']) && !demonRanks.contains(row['rank']) && !demonHealths.contains(row['health_points']) &&
              !demonDamages.contains(row['damage']) && !demonDescriptions.contains(row['description'])) {
            demonClasses.add(row['class']);
            demonRanks.add(row['rank']);
            demonHealths.add(row['health_points'] as int);
            demonDamages.add(row['damage'] as int);
            demonDescriptions.add(row['description']);
          }
        }
        ...
        for (Map<String, dynamic> row in images) {
          if (!demonImages.contains('./assets/demons/${row['content']}') &&!demonNames.contains(row['name'])) {
            demonImages.add('./assets/demons/${row['content']}');
            demonNames.add(row['name']);
          }
        }
        ...
        demons.add({
          'class': demonClasses,
          'rank': demonRanks,
          'health_points': demonHealths.map((e) => e.toString()).toList(),
          'damage': demonDamages.map((e) => e.toString()).toList(),
          'description': demonDescriptions,
          'images': demonImages,
          'names': demonNames,
        });
      }
  ...
  Future<void> initDatabase() async {
    ...
    await database.transaction((txn) async {
      ...
      await txn.rawInsert(
          'INSERT INTO images (id_character, id_demon, id_weapon, name, content) VALUES (?, ?, ?, ?, ?)',
          [null, 2, null, 'Imp (Doom Eternal)', "imp_4.jpg"]);
      await txn.rawInsert(
          'INSERT INTO images (id_character, id_demon, id_weapon, name, content) VALUES (?, ?, ?, ?, ?)',
      ...
      await txn.rawInsert(
          'INSERT INTO images (id_character, id_demon, id_weapon, name, content) VALUES (?, ?, ?, ?, ?)',
          [null, 8, null, 'Hell Knight (Doom Eternal)', "knight_4.jpg"]);
      await txn.rawInsert(
          'INSERT INTO images (id_character, id_demon, id_weapon, name, content) VALUES (?, ?, ?, ?, ?)',
      ...
      await txn.rawInsert(
          'INSERT INTO images (id_character, id_demon, id_weapon, name, content) VALUES (?, ?, ?, ?, ?)',
          [null, 10, null, 'Cyberdemon (Doom I - II)', "cyberdemon_1.png"]);
      await txn.rawInsert(
          'INSERT INTO images (id_character, id_demon, id_weapon, name, content) VALUES (?, ?, ?, ?, ?)',
          [null, 10, null, 'Cyberdemon (Doom 3)', "cyberdemon_2.png"]);
      await txn.rawInsert(
          'INSERT INTO images (id_character, id_demon, id_weapon, name, content) VALUES (?, ?, ?, ?, ?)',
          [null, 10, null, 'Cyberdemon (Doom)', "cyberdemon_3.png"]);
      await txn.rawInsert(
          'INSERT INTO images (id_character, id_demon, id_weapon, name, content) VALUES (?, ?, ?, ?, ?)',
          [null, 10, null, 'Cyberdemon (Doom Eternal)', "cyberdemon_4.png"]);
      await txn.rawInsert(
          'INSERT INTO images (id_character, id_demon, id_weapon, name, content) VALUES (?, ?, ?, ?, ?)',
